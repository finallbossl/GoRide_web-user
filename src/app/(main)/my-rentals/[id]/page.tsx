...
import { rentalApi, paymentApi, motorbikeApi } from '@/services/api';
import { RentalStatus, MotorbikeStatus } from '@goride/shared';
import {
  ArrowLeft,
  MapPin,
  Calendar,
...
export default function RentalDetailPage() {
  ...
  const handleCancelRental = async () => {
    if (!rental?.id || cancelLoading) return;

    const confirmed = window.confirm('Bạn có chắc muốn hủy đơn chưa thanh toán này? Xe sẽ được mở lại để khách khác đặt.');
    if (!confirmed) return;

    setCancelLoading(true);
    try {
      const cancelResponse = await rentalApi.updateStatus(rental.id, RentalStatus.CANCELLED);
      if (!cancelResponse.success) {
        throw new Error(cancelResponse.message || 'Không thể hủy đơn.');
      }

      const bikeId = rental.motorbikeId || rental.motorbike?.id;
      if (bikeId) {
        const bikeResponse = await motorbikeApi.updateStatus(bikeId, MotorbikeStatus.AVAILABLE);
        if (!bikeResponse.success) {
          throw new Error(bikeResponse.message || 'Đã hủy đơn nhưng chưa thể cập nhật trạng thái xe.');
        }
      }

      setRental((prev: any) => prev ? {
        ...prev,
        status: RentalStatus.CANCELLED,
        motorbike: prev.motorbike ? { ...prev.motorbike, status: MotorbikeStatus.AVAILABLE } : prev.motorbike,
      } : prev);
      setToast({ message: 'Hủy đơn thành công. Xe đã sẵn sàng để đặt lại.', type: 'success' });
      setTimeout(() => setToast(null), 5000);
    } catch (err: any) {
      setToast({ message: err?.message || 'Hủy đơn thất bại.', type: 'error' });
      setTimeout(() => setToast(null), 5000);
    } finally {
                  ...
                  <div className="mb-8">
                    <div className="p-8 bg-cta/5 rounded-[2.5rem] border border-cta/10 flex flex-col items-center gap-6 text-center shadow-inner-sm mb-6">
                      ...
                      <button
                        ...
                        className="w-full py-4 rounded-2xl border border-red-500/30 bg-red-50 text-red-600 text-[10px] font-black uppercase tracking-[0.3em] hover:bg-red-100 transition-all disabled:opacity-50"
                      >
                        {cancelLoading ? 'ĐANG HỦY ĐƠN...' : 'HỦY ĐƠN NGAY'}
                      </button>
                    </div>
